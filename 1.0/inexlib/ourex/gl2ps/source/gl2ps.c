...
/*G.Barrand :
  for this .c file, TARGET_OS_IPHONE does not seem to be defined
  through the included .h. We use the GL2PS_IPHONE_NDK which is set
  in Xcode C flags.
*/

#include "../gl2ps/ourex_gl2ps.h" /*G.Barrand*/
...
#ifdef OUREX_GL2PS_RENAME_GL /*G.Barrand*/
                          ...
                          GLshort fontsize, GLint alignment, GLfloat angle)
{
  GLfloat pos[4];
  GL2PSprimitive *prim;
  GLboolean valid = GL_FALSE; /*G.Barrand : init*/
  pos[0] = 0;pos[1] = 0;pos[2] = 0;pos[3] = 0; /*G.Barrand : init*/
...
{
  GLint index = 0; /*G.Barrand : init*/
  GLfloat rgba[4];
  int x = viewport[0], y = viewport[1], w = viewport[2], h = viewport[3];
  rgba[0] = 0;rgba[1] = 0;rgba[2] = 0;rgba[3] = 0; /*G.Barrand : init*/
...
/*G.Barrand */
GL2PSDLL_API GLint gl2psSetBackgroundColor(float a_r,float a_g,float a_b)
{
  if(!gl2ps) return GL2PS_UNINITIALIZED;
